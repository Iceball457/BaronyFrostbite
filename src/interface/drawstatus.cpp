					...
					DamageGib gibType)
{
	...
	if (player == -1)
	{
		if (source->behavior == &actMonster && source->monsterAllySummonRank != 0 && (target->behavior == &actMonster || target->behavior == &actPlayer))
		{
...
void drawHPMPBars(int player)
{
	...
	drawTooltip(&pos);
	if (stats[player] && stats[player]->HP > 0 && stats[player]->EFFECTS[EFF_HP_REGEN])
	{
	...
	pos.y = y2 - (playerStatusBarHeight + 12) - hpmpbarOffsets;
	drawTooltip(&pos);
	if (stats[player] && stats[player]->HP > 0 && stats[player]->EFFECTS[EFF_MP_REGEN])
	{
		bool lowDurationFlash = !((ticks % 50) - (ticks % 25));
...
void drawStatus(int player)
{
	...
	for (num = 0; num < NUM_HOTBAR_SLOTS; ++num)
	{
		...
		if (item)
		{
			...
			if (stats[player] && stats[player]->HP > 0)
			{
				...
				{
					if ((inputs.bMouseLeft(player) || (inputs.bControllerInputPressed(player, INJOY_MENU_LEFT_CLICK) && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !GenericGUI[player].isGUIOpen())) && !selectedItem)
					{
					...
					if (inputs.bMouseRight(player) || (inputs.bControllerInputPressed(player, INJOY_MENU_USE) && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !GenericGUI[player].isGUIOpen()))
					{
						...
						{
							if ((itemCategory(item) == POTION || itemCategory(item) == SPELLBOOK || item->type == FOOD_CREAMPIE) && (keystatus[SDLK_LALT] || keystatus[SDLK_RALT]))
							{
								badpotion = true;
								learnedSpell = true;
							}

							if (!learnedSpell && item->identified && itemCategory(item) == SPELLBOOK && players[player] && players[player]->entity)
							{
							...
							if (itemCategory(item) == SPELLBOOK && stats[player] && (stats[player]->type == GOBLIN || (stats[player]->playerRace == RACE_GOBLIN && stats[player]->appearance == 0)))
							{
							...
							if (!badpotion && !learnedSpell)
							{
								if (!(isItemEquippableInShieldSlot(item) && cast_animation[player].active_spellbook))
								{
									if (!disableItemUsage)
									{
										if (stats[player] && stats[player]->type == AUTOMATON && (item->type == TOOL_METAL_SCRAP || item->type == TOOL_MAGIC_SCRAP))
										{
											// consume item
			...
			if (!used)
			{
				...
				else
				{
					...
					if (players[player]->magic.selectedSpell() == spell && (players[player]->magic.selected_spell_last_appearance == item->appearance || players[player]->magic.selected_spell_last_appearance == -1))
					{
						// drawImageScaled(equipped_bmp, NULL, &src);
					}
				}
				...
				if (drawTooltipOnSlot)
				{
					...
					else
					{
						...
						if (item->identified)
						{
							...
							if (itemCategory(item) == WEAPON || itemCategory(item) == ARMOR || itemCategory(item) == THROWN || itemTypeIsQuiver(item->type))
							{
								src.h += TTF12_HEIGHT;
							}
							...
							else if (item->type == TOOL_GYROBOT || item->type == TOOL_DUMMYBOT || item->type == TOOL_SENTRYBOT || item->type == TOOL_SPELLBOT || (item->type == ENCHANTED_FEATHER && item->identified))
							{
								src.w += 7 * TTF12_WIDTH;
							}
						}
						...
						if (item->type == TOOL_GYROBOT || item->type == TOOL_DUMMYBOT || item->type == TOOL_SENTRYBOT || item->type == TOOL_SPELLBOT)
						{
							int health = 100;
						...
						{
							if (itemCategory(item) == WEAPON || itemCategory(item) == THROWN || itemTypeIsQuiver(item->type))
							{
								Monster tmpRace = stats[player]->type;
								if (stats[player]->type == TROLL || stats[player]->type == RAT || stats[player]->type == SPIDER || stats[player]->type == CREATURE_IMP)
								{
	...
	if (!command && stats[player] && stats[player]->HP > 0)
	{
			  ...
			  ))
		{
			...
			if (players[player]->hotbar.useHotbarFaceMenu && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !GenericGUI[player].isGUIOpen() && !inputs.getUIInteraction(player)->selectedItem && shootmode)
			{
				...
				if (pressed != Player::Hotbar_t::GROUP_NONE && players[player]->hotbar.faceMenuQuickCastEnabled && item && itemCategory(item) == SPELL_CAT)
				{
					spell_t *spell = getSpellFromItem(player, item);
		...
		if (!players[player]->hotbar.useHotbarFaceMenu && input.consumeBinaryToggle("Hotbar Right") && players[player]->bControlEnabled && !gamePaused && !players[player]->usingCommand())
		{
			if (shootmode && !inputs.getUIInteraction(player)->itemMenuOpen && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !players[player]->bookGUI.bBookOpen && !GenericGUI[player].isGUIOpen())
			{
				bumper_moved = true;
		...
		if (!players[player]->hotbar.useHotbarFaceMenu && input.consumeBinaryToggle("Hotbar Left") && players[player]->bControlEnabled && !gamePaused && !players[player]->usingCommand())
		{
			if (shootmode && !inputs.getUIInteraction(player)->itemMenuOpen && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !players[player]->bookGUI.bBookOpen && !GenericGUI[player].isGUIOpen())
			{
				bumper_moved = true;
		...
		if (bumper_moved && !inputs.getUIInteraction(player)->itemMenuOpen && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !players[player]->bookGUI.bBookOpen && !GenericGUI[player].isGUIOpen())
		{
		...
		{
			if (shootmode && input.consumeBinaryToggle("Hotbar Up / Select") && (!hotbar_t.useHotbarFaceMenu || (hotbar_t.useHotbarFaceMenu && !inputs.hasController(player))) && players[player]->bControlEnabled && !gamePaused && !players[player]->usingCommand() && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !players[player]->bookGUI.bBookOpen && !GenericGUI[player].isGUIOpen())
			{
				// Show a tooltip
		...
		if (item)
		{
			...
			if ((keystatus[SDLK_LALT] || keystatus[SDLK_RALT]) && (itemCategory(item) == POTION || itemCategory(item) == SPELLBOOK || item->type == FOOD_CREAMPIE))
			{
				badpotion = true;
				learnedSpell = true;
			}
			...
			{
				if (stats[player]->type == GOBLIN || stats[player]->type == CREATURE_IMP || (stats[player]->playerRace == RACE_GOBLIN && stats[player]->appearance == 0))
				{
					learnedSpell = true; // goblinos can't learn spells but always equip books.
				}
			}
	...
	if ((!shootmode || players[player]->characterSheet.lock_right_sidebar) && players[player]->characterSheet.proficienciesPage == 1 && pos.y < (players[player]->characterSheet.partySheetBox.y + players[player]->characterSheet.partySheetBox.h + 16))
	{
...
void drawStatusNew(const int player)
{
	...
	for (int num = 0; num < NUM_HOTBAR_SLOTS; ++num)
	{
		...
		if (item)
		{
			...
			if (stats[player] && stats[player]->HP > 0)
			{
				...
				if (!shootmode && !hotbarSlotFrame->isDisabled() && players[player]->GUI.bModuleAccessibleWithMouse(Player::GUI_t::MODULE_HOTBAR) && !selectedItem && players[player]->hotbar.current_hotbar == num && players[player]->GUI.activeModule == Player::GUI_t::MODULE_HOTBAR && !players[player]->GUI.isDropdownActive() && hotbarSlotFrame->capturesMouse() && players[player]->bControlEnabled && !gamePaused && !players[player]->usingCommand())
				{
					if ((Input::inputs[player].binaryToggle("MenuLeftClick") && inputs.bPlayerUsingKeyboardControl(player)) || (Input::inputs[player].binaryToggle(getContextMenuOptionBindingName(player, PROMPT_GRAB).c_str()) && hotbarGamepadControlEnabled(player)) && (players[player]->inventoryUI.bFirstTimeSnapCursor))
					{
						toggleclick = false;
						if ((keystatus[SDLK_LSHIFT] || keystatus[SDLK_RSHIFT]) && Input::inputs[player].binaryToggle("MenuLeftClick") && inputs.bPlayerUsingKeyboardControl(player))
						{
							hotbar[num].item = 0;
							hotbar[num].resetLastItem();
						...
						else
						{
							...
							if (Input::inputs[player].binaryToggle(getContextMenuOptionBindingName(player, PROMPT_GRAB).c_str()) && !openedChest[player] && gui_mode != (GUI_MODE_SHOP))
							{
					...
					if (Input::inputs[player].binaryToggle("MenuRightClick") && inputs.bPlayerUsingKeyboardControl(player) && !players[player]->GUI.isDropdownActive() && !selectedItem)
					{
	...
	if ((!shootmode && !FollowerMenu[player].followerMenuIsOpen()) || drawHotBarTooltipOnCycle)
	{
		...
		for (int num = 0; num < NUM_HOTBAR_SLOTS; ++num)
		{
			...
			if (item)
			{
				bool drawTooltipOnSlot = !shootmode && !hotbarSlotFrame->isDisabled() && players[player]->hotbar.current_hotbar == num && players[player]->GUI.activeModule == Player::GUI_t::MODULE_HOTBAR && players[player]->GUI.bModuleAccessibleWithMouse(Player::GUI_t::MODULE_HOTBAR) && hotbarSlotFrame->capturesMouse();
				if (!drawTooltipOnSlot)
				{
					if (drawHotBarTooltipOnCycle && players[player]->hotbar.current_hotbar == num)
					{
				...
				if (drawTooltipOnSlot)
				{
					...
					if (hotbar_t.hotbarFrame && players[player]->inventoryUI.tooltipFrame && !inputs.getUIInteraction(player)->selectedItem && !players[player]->GUI.isDropdownActive())
					{
						...
						if (players[player]->inventoryUI.tooltipPromptFrame && !players[player]->inventoryUI.tooltipPromptFrame->isDisabled() && !(players[player]->inventoryUI.useItemDropdownOnGamepad == Player::Inventory_t::GAMEPAD_DROPDOWN_FULL && !inputs.getVirtualMouse(player)->draw_cursor))
						{
						...
						if (players[player]->inventoryUI.tooltipPromptFrame)
						{
							...
							if (players[player]->inventoryUI.useItemDropdownOnGamepad == Player::Inventory_t::GAMEPAD_DROPDOWN_FULL && !inputs.getVirtualMouse(player)->draw_cursor)
							{
					...
					if (!drawHotBarTooltipOnCycle && playerSettings[multiplayer ? 0 : player].hotbar_numkey_quick_add && players[player]->bControlEnabled && !gamePaused && !players[player]->usingCommand() && inputs.bPlayerUsingKeyboardControl(player))
					{
						Uint32 swapItem = 0;
						...
						if (Input::inputs[player].binaryToggle("Hotbar Slot 10") && hotbar_t.getHotbarSlotFrame(9) && !hotbar_t.getHotbarSlotFrame(9)->isDisabled())
						{
				...
				{
					if (shootmode || (!shootmode && players[player]->GUI.activeModule == Player::GUI_t::MODULE_HOTBAR))
					{
						if (appraisal.itemsToNotify.find(item->uid) != appraisal.itemsToNotify.end())
						{
	...
	// NOTE: If you change the number of hotbar slots, you *MUST* change this.
	if (!players[player]->usingCommand() && stats[player] && stats[player]->HP > 0 && players[player]->bControlEnabled && !gamePaused)
	{
		Item *item = NULL;
		const auto &inventoryUI = players[player]->inventoryUI;
		if (inputs.bPlayerUsingKeyboardControl(player) && players[player]->gui_mode != GUI_MODE_SIGN && (shootmode || (!shootmode && !(playerSettings[multiplayer ? 0 : player].hotbar_numkey_quick_add && (mouseInsidePlayerHotbar(player) || mouseInsidePlayerInventory(player))))))
		{
			// if hotbar_numkey_quick_add is enabled, then the number keys won't do the default equip function
			// skips equipping items if the mouse is in the hotbar or inventory area. otherwise the below code runs.
			if (Input::inputs[player].binaryToggle("Hotbar Slot 1"))
		...
		// Moving the cursor changes the currently selected hotbar slot.
		if ((mousexrel || mouseyrel) && !shootmode && !players[player]->GUI.isDropdownActive() && players[player]->GUI.bModuleAccessibleWithMouse(Player::GUI_t::MODULE_HOTBAR))
		{
			if (hotbar_t.hotbarFrame)
			{
		...
		if (!players[player]->hotbar.useHotbarFaceMenu && Input::inputs[player].binaryToggle("Hotbar Right"))
		{
			...
			else if (shootmode && !players[player]->GUI.isDropdownActive() && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !players[player]->bookGUI.bBookOpen && !players[player]->signGUI.bSignOpen && !GenericGUI[player].isGUIOpen())
			{
		...
		{
			if ((!hotbar_t.useHotbarFaceMenu || (hotbar_t.useHotbarFaceMenu && !inputs.hasController(player))) && input.consumeBinaryToggle("Hotbar Up / Select") && shootmode && !openedChest[player] && gui_mode != (GUI_MODE_SHOP) && !players[player]->bookGUI.bBookOpen && !players[player]->signGUI.bSignOpen && !GenericGUI[player].isGUIOpen())
			{
				// Show a tooltip
				hotbar_t.hotbarTooltipLastGameTick = std::max(ticks - TICKS_PER_SECOND, ticks - hotbar_t.hotbarTooltipLastGameTick);
			...
			if (!shootmode && inventoryInteractable && !players[player]->bookGUI.bBookOpen && !players[player]->signGUI.bSignOpen && !openedChest[player] && mouseInsidePlayerHotbar(player))
			{
				if (tooltipOpen && !tooltipPromptFrameWasDisabled)
				{
					item = uidToItem(hotbar[hotbar_t.current_hotbar].item);
