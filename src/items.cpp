...
void autoHotbarTryAdd(const int player, Item &item)
{
	...
	Category cat = itemCategory(&item);
	if (!item.identified && cat != SPELL_CAT)
	{
		return;
	}
	...
	if (softReserveSlots)
	{
		for (auto &slot : slots)
		{
			...
			if (!hotbar_slot->lastItem.identified)
			{
				continue;
			} // no good
...
Item *newItem(const ItemType type, const Status status, const Sint16 beatitude, const Sint16 count, const Uint32 appearance, const bool identified, list_t *const inventory)
{
	...
	item->status = (int)status < Status::BROKEN ? Status::BROKEN : ((int)status > EXCELLENT ? EXCELLENT : status);
...
int itemCompare(const Item *const item1, const Item *const item2, bool checkAppearance, bool comparisonUsedForStacking)
{
	...
	else if (item1->type == SCROLL_MAIL || item1->type == READABLE_BOOK || items[item1->type].category == SPELL_CAT || item1->type == TOOL_PLAYER_LOOT_BAG)
	{
...
Entity *dropItemMonster(Item *const item, Entity *const monster, Stat *const monsterStats, Sint16 count)
{
	...
	if (monsterStats)
	{
		...
		{
			if (monster->behavior == &actMonster && (item->type < ARTIFACT_ORB_BLUE || item->type > ARTIFACT_ORB_GREEN))
			{
			...
			else
			{
				...
				if ((monsterStats->type == KOBOLD || monsterStats->type == COCKATRICE || monsterStats->type == INSECTOID || monsterStats->type == INCUBUS || monsterStats->type == VAMPIRE || monsterStats->type == SUCCUBUS) && (itemCategory(item) == SPELLBOOK || itemCategory(item) == MAGICSTAFF))
				{
	...
	if (itemDroppable)
	{
		...
		if (monsterStats)
		{
			...
			else if (item->type == ARTIFACT_ORB_PURPLE && monsterStats->type == LICH)
			{
				...
				if (map.tiles[OBSTACLELAYER + iy * MAPLAYERS + ix * MAPLAYERS * map.height] || !map.tiles[iy * MAPLAYERS + ix * MAPLAYERS * map.height])
				{
...
EquipItemResult equipItem(Item *const item, Item **const slot, const int player, bool checkInventorySpaceForPaperDoll)
{
	...
	if (players[player]->isLocalPlayer() && multiplayer != SINGLE && item->unableToEquipDueToSwapWeaponTimer(player))
	{
		return EQUIP_ITEM_FAIL_CANT_UNEQUIP;
	}
		...
		if (multiplayer != CLIENT && !intro && !fadeout)
		{
			if (players[player] != nullptr && players[player]->entity != nullptr)
			{
				if (players[player]->entity->ticks > 60)
				{
					...
					else if (itemCategory(item) == ARMOR || item->type == TOOL_TINKERING_KIT || itemTypeIsQuiver(item->type))
					{
	...
	else
	{
		...
		if (*slot != nullptr)
		{
			if ((*slot)->count == item->count) // if quantity is the same then it's the same item, can unequip
			{
				...
				if (players[player]->isLocalPlayer() && players[player]->paperDoll.enabled && players[player]->paperDoll.isItemOnDoll(**slot))
				{
...
void useItem(Item *item, const int player, Entity *usedBy, bool unequipForDropping)
{
	...
	{
		if (itemCategory(item) == POTION && item->type != POTION_EMPTY && usedBy && (players[player] && players[player]->entity) && players[player]->entity == usedBy)
		{
			if (item->identified)
			{
	...
	case AMULET_STRANGULATION:
	{
		...
		equipItemResult = equipItem(item, &stats[player]->amulet, player, checkInventorySpaceForPaperDoll);
		if (stats[player]->amulet && stats[player]->amulet->type == AMULET_STRANGULATION && !oldStrangulation)
		{
			if (players[player]->isLocalPlayer())
			{
	...
	case TOOL_PLAYER_LOOT_BAG:
		if (multiplayer != CLIENT)
		{
			...
			if (lootbagPlayer >= 0 && lootbagPlayer < MAXPLAYERS && stats[lootbagPlayer])
			{
	...
	if (players[player]->isLocalPlayer())
	{
		...
		if (drankPotion && usedBy && (players[player] && players[player]->entity) && players[player]->entity == usedBy)
		{
...
Item *itemPickup(const int player, Item *const item, Item *addToSpecificInventoryItem, bool forceNewStack)
{
	...
	else
	{
		...
		for (node_t *node = stats[player]->inventory.first; node != nullptr; node = node->next)
		{
			...
			{
				if ((itemTypeIsQuiver(item2->type) && (item->count + item2->count) >= QUIVER_MAX_AMMO_QTY) || ((item2->type == TOOL_MAGIC_SCRAP || item2->type == TOOL_METAL_SCRAP) && (item->count + item2->count) >= SCRAP_MAX_STACK_QTY))
				{
		...
		if (!appearancesOfSimilarItems.empty() && item && item->type >= 0 && item->type < NUMITEMS)
		{
			...
			if (item->type == TOOL_SENTRYBOT || item->type == TOOL_SPELLBOT || item->type == TOOL_GYROBOT || item->type == TOOL_DUMMYBOT)
			{
				robot = true;
...
ItemStackResult getItemStackingBehaviorIndividualItemCheck(const int player, Item *itemToCheck, Item *itemDestinationStack, int &newQtyForCheckedItem, int &newQtyForDestItem)
{
	...
	{
		if ((itemTypeIsQuiver(itemDestinationStack->type) && (itemToCheck->count + itemDestinationStack->count) >= QUIVER_MAX_AMMO_QTY) || ((itemDestinationStack->type == TOOL_MAGIC_SCRAP || itemDestinationStack->type == TOOL_METAL_SCRAP) && (itemToCheck->count + itemDestinationStack->count) >= SCRAP_MAX_STACK_QTY))
		{
			int maxStack = QUIVER_MAX_AMMO_QTY;
...
void Item::applyLockpickToWall(const int player, const int x, const int y) const
{
	...
	for (node_t *node = map.entities->first; node != nullptr; node = node->next)
	{
		...
		if (entity && entity->behavior == &actArrowTrap && static_cast<int>(entity->x / 16) == x && static_cast<int>(entity->y / 16) == y)
		{
			// found a trap.
			if (entity->skill[4] == 0)
			{
				if (player >= 0 && players[player] && players[player]->entity && stats[player] && stats[player]->weapon && (stats[player]->weapon->type == TOOL_LOCKPICK || stats[player]->weapon->type == TOOL_SKELETONKEY))
				{
...
bool isPotionBad(const Item &potion)
{
	...
	if (potion.identified &&
		(potion.type == POTION_SICKNESS || potion.type == POTION_CONFUSION || potion.type == POTION_BLINDNESS || potion.type == POTION_ACID || potion.type == POTION_PARALYSIS || potion.type == POTION_FIRESTORM || potion.type == POTION_ICESTORM || potion.type == POTION_THUNDERSTORM))
	{
		return true;
	}
...
bool Item::shouldItemStack(const int player, bool ignoreStackLimit) const
{
	if (player >= 0)
	{
		if ((!itemIsEquipped(this, player) && itemCategory(this) != ARMOR && itemCategory(this) != WEAPON && itemCategory(this) != MAGICSTAFF && itemCategory(this) != RING && itemCategory(this) != AMULET && itemCategory(this) != SPELLBOOK && this->type != TOOL_PICKAXE && this->type != TOOL_ALEMBIC && this->type != TOOL_TINKERING_KIT && this->type != ENCHANTED_FEATHER && this->type != TOOL_LANTERN && this->type != TOOL_GLASSES && this->type != TOOL_MIRROR && this->type != TOOL_BLINDFOLD && this->type != TOOL_BLINDFOLD_FOCUS && this->type != TOOL_BLINDFOLD_TELEPATHY) || itemCategory(this) == THROWN || itemCategory(this) == GEM || itemCategory(this) == POTION || (itemCategory(this) == TOOL && this->type != TOOL_PICKAXE && this->type != TOOL_ALEMBIC && this->type != TOOL_TINKERING_KIT && this->type != ENCHANTED_FEATHER && this->type != TOOL_LANTERN && this->type != TOOL_GLASSES && this->type != TOOL_MIRROR && this->type != TOOL_BLINDFOLD && this->type != TOOL_BLINDFOLD_FOCUS && this->type != TOOL_BLINDFOLD_TELEPATHY))
		{
...
bool itemSpriteIsQuiverThirdPersonModel(const int sprite)
{
	...
	{
		if (sprite == items[i].index || sprite == items[i].index + 1 || sprite == items[i].index + 2 || sprite == items[i].index + 3)
		{
			return true;
		}
	}
...
bool itemSpriteIsBreastpiece(const int sprite)
{
	...
	if (sprite == items[LEATHER_BREASTPIECE].index || sprite == items[IRON_BREASTPIECE].index || sprite == items[STEEL_BREASTPIECE].index || sprite == items[CRYSTAL_BREASTPIECE].index || sprite == items[VAMPIRE_DOUBLET].index || sprite == items[WIZARD_DOUBLET].index || sprite == items[HEALER_DOUBLET].index || sprite == items[SILVER_DOUBLET].index || sprite == items[ARTIFACT_BREASTPIECE].index || sprite == items[TUNIC].index || sprite == items[MACHINIST_APRON].index)
	{
		return true;
	}
	return false;
...
bool playerCanSpawnMoreTinkeringBots(const Stat *const myStats)
{
	...
	for (node_t *node = myStats->FOLLOWERS.first; node != nullptr; node = node->next)
	{
		...
		if (follower)
		{
			...
			if (followerStats)
			{
				if (followerStats->type == SENTRYBOT || followerStats->type == GYROBOT || followerStats->type == SPELLBOT || followerStats->type == DUMMYBOT)
				{
					++numBots;
				}
			}
...
void playerTryEquipItemAndUpdateServer(const int player, Item *const item, bool checkInventorySpaceForPaperDoll)
{
	...
	{
		if (players[player]->isLocalPlayer() && players[player]->paperDoll.enabled && players[player]->paperDoll.isItemOnDoll(*item))
		{
...
int Item::getLootBagNumItems() const
{
	...
	if (stats[0]->player_lootbags.find(appearance) != stats[0]->player_lootbags.end())
	{
