...
bool item_PotionWater(Item *&item, Entity *entity, Entity *usedBy)
{
	...
	{
		if (stats->amulet->type == AMULET_STRANGULATION && stats->type != SKELETON)
		{
	...
	else if (item->beatitude < 0)
	{
		...
		while (!toCurse && !breakloop)
		{
			...
			case 0:
				if (stats->weapon != nullptr && itemCategory(stats->weapon) != POTION && stats->weapon->beatitude >= 0)
				{
...
bool item_PotionSickness(Item *&item, Entity *entity, Entity *usedBy)
{
	...
	if (stats->type == LICH || stats->type == SHOPKEEPER || stats->type == DEVIL || stats->type == MINOTAUR || stats->type == LICH_FIRE || stats->type == LICH_ICE)
	{
...
bool item_PotionUnstableStorm(Item *&item, Entity *entity, Entity *usedBy, Entity *thrownPotion)
{
	...
	if (item->type == POTION_FIRESTORM && !thrownPotion && usedBy && usedBy == entity && usedBy->behavior == &actPlayer && stats->type == AUTOMATON)
	{
...
void item_ToolTowel(Item *&item, int player)
{
	...
	if (multiplayer != CLIENT)
	{
		if (stats[player]->EFFECTS[EFF_GREASY] || stats[player]->EFFECTS[EFF_MESSY] || stats[player]->EFFECTS[EFF_BLEEDING])
		{
...
void item_Food(Item *&item, int player)
{
	...
	{
		if (stats[player]->amulet->type == AMULET_STRANGULATION && stats[player]->type != SKELETON)
		{
			if (players[player]->isLocalPlayer())
			{
	...
	if (players[player]->isLocalPlayer())
	{
		...
		if (stats[player]->playerRace == RACE_SKELETON && stats[player]->appearance == 0 && players[player] && players[player]->entity->effectPolymorph > NUMMONSTERS)
		{
	...
	if (players[player] && players[player]->entity && playerRequiresBloodToSustain(player))
	{
		if (item->type == FOOD_BLOOD)
		{
			pukeChance = 100;
	...
	if (((item->beatitude < 0 && item->type != FOOD_CREAMPIE) || (local_rng.rand() % pukeChance == 0)) && pukeChance < 100)
	{
		...
		{
			if (stats[player]->type != SKELETON && players[player]->entity->effectShapeshift == NOTHING && stats[player]->type != AUTOMATON)
			{
...
void item_AmuletSexChange(Item *item, int player)
{
	...
	if (stats[player]->sex == FEMALE && stats[player]->playerRace == RACE_INCUBUS && stats[player]->appearance == 0)
	{
		messagePlayer(player, MESSAGE_HINT, Language::get(4048)); // don't feel like yourself
	}
	else if (stats[player]->sex == MALE && stats[player]->playerRace == RACE_SUCCUBUS && stats[player]->appearance == 0)
	{
		messagePlayer(player, MESSAGE_HINT, Language::get(4048)); // don't feel like yourself
	}
	else
...
void item_FoodAutomaton(Item *&item, int player)
{
	...
	if (!itemIsConsumableByAutomaton(*item) && item->type != POTION_FIRESTORM)
	{
		return;
	}
