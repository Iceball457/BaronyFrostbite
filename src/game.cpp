...
void TimerExperiments::updateEntityInterpolationPosition(Entity *entity)
{
	...
	if (entity->behavior == &actHudWeapon || entity->behavior == &actHudShield || entity->behavior == &actHudArm || entity->behavior == &actHudAdditional || entity->behavior == &actHudArrowModel || entity->behavior == &actLeftHandMagic || entity->behavior == &actRightHandMagic || entity->behavior == &actDoor)
	{
...
void gameLogic(void)
{
	...
	else
	{
		...
		if (multiplayer != CLIENT) // server/singleplayer code
		{
			...
			if (ticks % TICKS_PER_SECOND == 0)
			{
				for (c = 0; c < MAXPLAYERS; c++)
				{
					...
					if (stats[c]->helmet && stats[c]->helmet->type == ARTIFACT_HELM && stats[c]->breastplate && stats[c]->breastplate->type == ARTIFACT_BREASTPIECE && stats[c]->gloves && stats[c]->gloves->type == ARTIFACT_GLOVES && stats[c]->cloak && stats[c]->cloak->type == ARTIFACT_CLOAK && stats[c]->shoes && stats[c]->shoes->type == ARTIFACT_BOOTS)
					{
						steamAchievementClient(c, "BARONY_ACH_GIFTS_ETERNALS");
					}

					if (stats[c]->type == SKELETON && stats[c]->weapon && stats[c]->weapon->type == ARTIFACT_AXE && stats[c]->cloak && stats[c]->cloak->type == CLOAK_PROTECTION && !stats[c]->gloves && !stats[c]->helmet && !stats[c]->shoes && !stats[c]->breastplate && !stats[c]->mask && !stats[c]->ring && !stats[c]->amulet && !stats[c]->shield)
					{
						// nothing but an axe and a cloak.
						steamAchievementClient(c, "BARONY_ACH_COMEDIAN");
					}

					if (stats[c]->EFFECTS[EFF_SHRINE_RED_BUFF] && stats[c]->EFFECTS[EFF_SHRINE_GREEN_BUFF] && stats[c]->EFFECTS[EFF_SHRINE_BLUE_BUFF])
					{
						...
						if (follower)
						{
							...
							if (followerStats)
							{
								...
								if (followerStats->type == GYROBOT || followerStats->type == SENTRYBOT || followerStats->type == SPELLBOT)
								{
									...
									{
										if (follower->monsterTarget != 0 && (follower->monsterState == MONSTER_STATE_ATTACK || follower->monsterState == MONSTER_STATE_HUNT) &&
			...
			for (node = map.entities->first; node != nullptr; node = nextnode)
			{
				...
				if (entity && !entity->ranbehavior)
				{
					...
					if (entity->behavior != nullptr)
					{
						if (gameloopFreezeEntities && entity->behavior != &actPlayer && entity->behavior != &actPlayerLimb && entity->behavior != &actHudWeapon && entity->behavior != &actHudShield && entity->behavior != &actHudAdditional && entity->behavior != &actHudArrowModel && entity->behavior != &actLeftHandMagic && entity->behavior != &actRightHandMagic && entity->behavior != &actFlame)
						{
						...
						if (entitiesdeleted.first != nullptr)
						{
							...
							if (entitydeletedself == false)
							{
								if (ox != -1 && oy != -1)
								{
									if (ox != static_cast<int>(entity->x) >> 4 || oy != static_cast<int>(entity->y) >> 4)
									{
				...
				if (loadnextlevel == true)
				{
					...
					for (c = 0; c < MAXPLAYERS; c++)
					{
						if (players[c] && players[c]->entity && !client_disconnected[c])
						{
							...
							{
								Stat *tempStats = (Stat *)node->element;
								if (tempStats && (tempStats->type == DUMMYBOT || tempStats->type == SENTRYBOT || tempStats->type == SPELLBOT))
								{
			...
			if (multiplayer == SERVER)
			{
				...
				else if ((ticks % (TICKS_PER_SECOND * 3) == 0) && (serverLastPlayerHealthUpdate == 0 || ((ticks - serverLastPlayerHealthUpdate) >= 2 * TICKS_PER_SECOND)))
				{
			...
			for (int player = 0; player < MAXPLAYERS; ++player)
			{
				...
				if (stats[player]->cloak && stats[player]->cloak->type == CLOAK_BACKPACK && stats[player]->cloak->status != BROKEN && (shouldInvertEquipmentBeatitude(stats[player]) ? abs(stats[player]->cloak->beatitude) >= 0 : stats[player]->cloak->beatitude >= 0))
				{
			...
			for (int player = 0; player < MAXPLAYERS; ++player)
			{
				...
				for (node = stats[player]->inventory.first; node != NULL; node = nextnode)
				{
					...
					if (item->status == BROKEN && itemCategory(item) != SPELL_CAT && item->x == Player::PaperDoll_t::ITEM_PAPERDOLL_COORDINATE)
					{
					...
					if (itemCategory(item) != SPELL_CAT && item->x != Player::PaperDoll_t::ITEM_PAPERDOLL_COORDINATE && item->x != Player::PaperDoll_t::ITEM_RETURN_TO_INVENTORY_COORDINATE && (item->x >= players[player]->inventoryUI.getSizeX() || item->y >= backpack_sizey[player]))
					{
					...
					{
						if (auto_appraise_new_items && players[player]->inventoryUI.appraisal.timer == 0 && !(item->identified) && players[player]->inventoryUI.appraisal.appraisalPossible(item))
						{
		...
		else if (multiplayer == CLIENT)
		{
			...
			if (stats[clientnum]->cloak && stats[clientnum]->cloak->type == CLOAK_BACKPACK && stats[clientnum]->cloak->status != BROKEN && (shouldInvertEquipmentBeatitude(stats[clientnum]) ? abs(stats[clientnum]->cloak->beatitude) >= 0 : stats[clientnum]->cloak->beatitude >= 0))
			{
			...
			for (node = stats[clientnum]->inventory.first; node != NULL; node = nextnode)
			{
				...
				// drop any inventory items you don't have room for
				if (itemCategory(item) != SPELL_CAT && item->x != Player::PaperDoll_t::ITEM_PAPERDOLL_COORDINATE && item->x != Player::PaperDoll_t::ITEM_RETURN_TO_INVENTORY_COORDINATE && (item->x >= players[clientnum]->inventoryUI.getSizeX() || item->y >= backpack_sizey))
				{
				...
				{
					if (auto_appraise_new_items && players[clientnum]->inventoryUI.appraisal.timer == 0 && !(item->identified) && players[clientnum]->inventoryUI.appraisal.appraisalPossible(item))
					{
	...
	while (SDL_PollEvent(&event)) // poll SDL events
	{
		...
		case SDL_KEYDOWN: // if a key is pressed...
...
#endif
			{
				...
				case SDLK_NUMLOCKCLEAR:
					lastkeypressed = SDLK_KP_CLEAR;
					break;
				case SDLK_PRINTSCREEN:
					lastkeypressed = SDLK_F13;
					break;
				case SDLK_SCROLLLOCK:
					lastkeypressed = SDLK_F14;
					break;
				case SDLK_PAUSE:
					lastkeypressed = SDLK_F15;
					break;
				}
#endif
		...
		case SDL_CONTROLLERBUTTONDOWN: // if joystick button is pressed
		{
			...
			case SDL_CONTROLLER_BUTTON_A:
				snprintf(buf, sizeof(buf), "Pad%dButtonA", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_B:
				snprintf(buf, sizeof(buf), "Pad%dButtonB", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_X:
				snprintf(buf, sizeof(buf), "Pad%dButtonX", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_Y:
				snprintf(buf, sizeof(buf), "Pad%dButtonY", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_BACK:
				snprintf(buf, sizeof(buf), "Pad%dButtonBack", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_START:
				snprintf(buf, sizeof(buf), "Pad%dButtonStart", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_LEFTSTICK:
				snprintf(buf, sizeof(buf), "Pad%dButtonLeftStick", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_RIGHTSTICK:
				snprintf(buf, sizeof(buf), "Pad%dButtonRightStick", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_LEFTSHOULDER:
				snprintf(buf, sizeof(buf), "Pad%dButtonLeftBumper", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_RIGHTSHOULDER:
				snprintf(buf, sizeof(buf), "Pad%dButtonRightBumper", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_DPAD_LEFT:
				snprintf(buf, sizeof(buf), "Pad%dDpadX-", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_DPAD_RIGHT:
				snprintf(buf, sizeof(buf), "Pad%dDpadX+", event.cbutton.which);
				break;
			case SDL_CONTROLLER_BUTTON_DPAD_UP:
				snprintf(buf, sizeof(buf), "Pad%dDpadY-", event.cbutton.which);
				break;
		...
		case SDL_JOYHATMOTION:
		{
			...
			if (Input::joysticks.find(event.jdevice.which) != Input::joysticks.end())
			{
				...
				{
				case SDL_HAT_LEFTUP:
					snprintf(buf, sizeof(buf), "Joy%dHat%dLeftUp", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_UP:
					snprintf(buf, sizeof(buf), "Joy%dHat%dUp", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_RIGHTUP:
					snprintf(buf, sizeof(buf), "Joy%dHat%dRightUp", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_RIGHT:
					snprintf(buf, sizeof(buf), "Joy%dHat%dRight", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_RIGHTDOWN:
					snprintf(buf, sizeof(buf), "Joy%dHat%dRightDown", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_DOWN:
					snprintf(buf, sizeof(buf), "Joy%dHat%dDown", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_LEFTDOWN:
					snprintf(buf, sizeof(buf), "Joy%dHat%dLeftDown", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_LEFT:
					snprintf(buf, sizeof(buf), "Joy%dHat%dLeft", event.jhat.which, event.jhat.hat);
					break;
				case SDL_HAT_CENTERED:
					snprintf(buf, sizeof(buf), "Joy%dHat%dCentered", event.jhat.which, event.jhat.hat);
					break;
				}
				Input::lastInputOfAnyKind = buf;
			}
...
void ingameHud()
{
	for (int player = 0; player < MAXPLAYERS; ++player)
	{
		...
		if (!gamePaused && bControlEnabled && !players[player]->usingCommand())
		{
			...
			if (players[player]->hotbar.faceMenuButtonHeld == Player::Hotbar_t::GROUP_NONE)
			{
				if (players[player]->bUseCompactGUIHeight() && players[player]->bUseCompactGUIWidth())
				{
					...
					if (players[player]->worldUI.isEnabled() && players[player]->worldUI.bTooltipInView && players[player]->worldUI.tooltipsInRange.size() > 1)
					{
						...
						if (expandBinding == cycleNextBinding || expandBinding == cyclePrevBinding)
						{
				...
				else if (players[player]->shootmode && players[player]->minimap.bExpandPromptEnabled && input.consumeBinaryToggle("Toggle Minimap"))
				{
					openMinimap(player);
				}
			}
		...
		// inventory interface
		// player not needed to be alive
		if (players[player]->isLocalPlayer() && !players[player]->usingCommand() && input.consumeBinaryToggle("Character Status") && !gamePaused && bControlEnabled)
		{
		...
		if (players[player]->isLocalPlayer() && !players[player]->shootmode && (players[player]->inventoryUI.useItemDropdownOnGamepad != Player::Inventory_t::GAMEPAD_DROPDOWN_DISABLE) && !inputs.getVirtualMouse(player)->draw_cursor && !players[player]->usingCommand() && input.binaryToggle("MenuCancel") && !players[player]->GUI.isDropdownActive() && players[player]->GUI.bModuleAccessibleWithMouse(players[player]->GUI.activeModule) && !inputs.getUIInteraction(player)->selectedItem && !gamePaused && bControlEnabled && players[player]->gui_mode == GUI_MODE_INVENTORY && players[player]->inventory_mode == INVENTORY_MODE_ITEM && !players[player]->inventoryUI.chestGUI.bOpen && !players[player]->minimap.mapWindow && !players[player]->messageZone.logWindow && !players[player]->shopGUI.bOpen && !GenericGUI[player].isGUIOpen())
		{
			if ((players[player]->inventoryUI.isInteractable && players[player]->GUI.activeModule == Player::GUI_t::MODULE_INVENTORY) || players[player]->GUI.activeModule == Player::GUI_t::MODULE_HOTBAR || players[player]->GUI.activeModule == Player::GUI_t::MODULE_CHARACTERSHEET)
			{
				players[player]->closeAllGUIs(CLOSEGUI_ENABLE_SHOOTMODE, CLOSEGUI_CLOSE_ALL);
		...
		// spell list
		// player not needed to be alive
		if (players[player]->isLocalPlayer() && !players[player]->usingCommand() && input.consumeBinaryToggle("Spell List") && !gamePaused && bControlEnabled) // TODO: Move to function in interface or something?
		{
			...
			else if (!inputs.getUIInteraction(player)->selectedItem && !players[player]->GUI.isDropdownActive() && !GenericGUI[player].isGUIOpen() && !inputs.hasController(player))
			{
		...
		if (players[player]->isLocalPlayerAlive() && !gamePaused)
		{
			...
			else if (!players[player]->usingCommand() && bControlEnabled && ((shootmode && inputs.hasController(player)) || (!inputs.hasController(player) && inputs.bPlayerUsingKeyboardControl(player))))
			{
		...
		bool worldUIBlocksFollowerCycle = (players[player]->worldUI.isEnabled() && players[player]->worldUI.bTooltipInView && players[player]->worldUI.tooltipsInRange.size() > 1);
		if (worldUIBlocksFollowerCycle)
		{
			std::string cycleNPCbinding = input.binding("Cycle NPCs");
			if (cycleNPCbinding != input.binding("Interact Tooltip Next") && cycleNPCbinding != input.binding("Interact Tooltip Prev"))
			{
		...
		if (!players[player]->usingCommand() && input.consumeBinaryToggle("Cycle NPCs") && !gamePaused && bControlEnabled)
		{
			...
			if (inputs.getUIInteraction(player)->selectedItem)
			{
				...
				if (frame)
				{
					...
					if (*cvar_debugVMouse)
					{
					 ...
					 (followerMenu.optionSelected == ALLY_CMD_MOVETO_SELECT || followerMenu.optionSelected == ALLY_CMD_ATTACK_SELECT))
			{
	...
	if (playercount >= 1)
	{
		...
		for (int c = 0; c < MAXPLAYERS; ++c)
		{
			...
			if (players[c] && players[c]->entity)
			{
				...
				{
					if (globalLightModifierActive == GLOBAL_LIGHT_MODIFIER_STOPPED || (globalLightModifierActive == GLOBAL_LIGHT_MODIFIER_DISSIPATING && globalLightModifier < 1.f))
					{
					...
					int PERModifier = 0;
					if (stats[c] && stats[c]->EFFECTS[EFF_BLIND] && !stats[c]->EFFECTS[EFF_ASLEEP] && !stats[c]->EFFECTS[EFF_MESSY])
					{
